...
Summaries are generated by the local **Claude Code CLI in headless mode** (`claude -p`), which authenticates via the Claude **Max subscription** rather than a metered Anthropic API key. All items are summarized in a single batched call; `ANTHROPIC_API_KEY` is stripped from the subprocess env so the CLI can't fall back to API credits.
